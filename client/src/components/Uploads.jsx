...
export const Uploads = ({isLive, isPost, isReel}) => {
  const {setUploadPopup, getData, postDatas} = useAppContext();
  ...
  useEffect(()=>{postDatas&&setPostID(postDatas.length+1) }, [])
  ...
  return (
    <div className="upload-container" >
        ...
        <div className="upload-profile"><BarLink photo={datas.admin.profile} name={localStorage.getItem('firstName')+" "+localStorage.getItem('lastName')}  isPost={true} access={<AccessTag accessIcon={<FaUserFriends />} accessTitle={"Friends"} changeIcon={<IoMdArrowDropdown />} />}/>
        </div>
        <form onSubmit={handlePostSubmit}>
          <div className="write-something">
            <input type="text" className='write-input' placeholder={`What's on your mind, your name`} onChange={(e)=>setCaption(e.target.value)}/>
